<?php
// CBE LMS — public map of school deployments (Kenya CBE curriculum).
// Reads cbe_devices, which cbe-sync.php populates from each school box.
// No auth: matches the sibling MTTI locations.php pattern.
...
function milestone(int $n):?string{
    if($n>=500)return '🏆';if($n>=200)return '🌳';if($n>=100)return '🌿';if($n>=25)return '🌱';return null;
}

$groups=[];$markers=[];$nowTs=time();
$totalDevices=0;$totalLearners=0;$totalLessons=0;$totalCerts=0;$onlineCount=0;
$weightedScoreNum=0.0;$weightedScoreDen=0;

if(!$dbError){
    $r=$m->query("SELECT device_id,school_name,county,region,lat,lng,
                         learner_count,learner_count_prev,lesson_completions,
                         quiz_attempts,avg_score,cert_count,active_last_30,
                         avg_sync_interval_secs,last_sync_at,app_version
                  FROM cbe_devices WHERE last_sync_at IS NOT NULL
    ...
    if($r) while($row=$r->fetch_assoc()){
        ...
        if($lat&&$lng){
            $markers[]=[
                'name'=>$row['school_name'],'group'=>$group,'county'=>$row['county'],
...
foreach($groups as $gName=>$devs){
    $rl=['learners'=>0,'certs'=>0,'devices'=>count($devs),'online'=>0,'score_sum'=>0,'score_n'=>0];
    foreach($devs as $d){
        $rl['learners']+=(int)$d['learner_count'];$rl['certs']+=(int)$d['cert_count'];
    ...
    $rl['cert_rate']=$rl['learners']>0?round($rl['certs']/$rl['learners']*100,1):null;
...
<title>CBE LMS — School Deployments Map</title>
<link rel="stylesheet" href="/leaflet.css">
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Segoe UI',Roboto,Arial,sans-serif;background:#f5f7fa;color:#1f2937;line-height:1.5;}
header{background:linear-gradient(135deg,#065f46,#059669);color:#fff;padding:24px 20px;text-align:center;}
...
.kpi .num{font-size:1.7rem;font-weight:800;color:#065f46;}
...
.toolbar input:focus{border-color:#059669;}
...
.view-btn.active{background:#065f46;color:#fff;}
...
.group-head h2{font-size:1.1rem;color:#065f46;}
.cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:14px;}
.card{background:#fff;border-radius:10px;padding:16px;border-left:4px solid #059669;box-shadow:0 1px 3px rgba(0,0,0,.05);transition:box-shadow .15s;}
...
.tag-region{background:#d1fae5;color:#065f46;font-size:.7rem;font-weight:700;padding:2px 8px;border-radius:8px;}
.cert-rate{display:inline-flex;align-items:center;gap:4px;background:#ecfdf5;color:#047857;font-size:.75rem;font-weight:700;padding:2px 8px;border-radius:8px;}
.metric{display:flex;justify-content:space-between;padding:3px 0;font-size:.85rem;}
.metric .lbl{color:#555;}
.metric .val{font-weight:700;color:#065f46;}
...
.rollup-card{background:#fff;border-radius:10px;padding:16px;box-shadow:0 1px 3px rgba(0,0,0,.05);border-top:4px solid #065f46;}
.rollup-card h3{font-size:1.05rem;color:#065f46;margin-bottom:12px;}
...
.rs .rv{font-size:1.2rem;font-weight:800;color:#065f46;}
...
.leaflet-popup-content h4{font-size:1rem;color:#065f46;margin-bottom:6px;}
.pop-row{display:flex;justify-content:space-between;gap:14px;font-size:.84rem;padding:2px 0;}
.pop-row .l{color:#555;}.pop-row .v{font-weight:700;color:#065f46;}
.pop-score{margin:6px 0 2px;padding:6px 8px;background:#ecfdf5;border-radius:6px;font-size:.82rem;}
@media(max-width:600px){#map{height:340px;}.kpi .num{font-size:1.4rem;}.rollup-stats{grid-template-columns:repeat(2,1fr);}}
</style>
</head>
<body>
<header>
  <h1>🏫 CBE LMS — School Deployments</h1>
  <p>Competency-Based Education · live map of school LAN boxes across Kenya</p>
</header>

<section class="kpis">
  <div class="kpi"><div class="num"><?= $totalDevices ?></div><div class="lbl">Schools</div></div>
...
<div class="toolbar">
  <input type="search" id="search" placeholder="🔍  Search schools..." oninput="filterCards()">
  <div class="view-btns">
    <button class="view-btn active" id="btn-devices" onclick="setView('devices')">Schools</button>
...
<?php if(!$groups): ?>
  <div class="empty">No school boxes have synced yet. Once a box runs <code>php artisan cbe:sync-cloud</code>, it will show here.</div>
<?php else: foreach($groups as $groupName=>$devs): ?>
  <div class="group" data-group="<?= h($groupName) ?>">
    <div class="group-head">
      <h2><?= h($groupName) ?></h2>
      <span style="font-size:.8rem;color:#9ca3af;">· <?= count($devs) ?> school<?= count($devs)!==1?'s':'' ?></span>
    </div>
    <div class="cards">
    <?php foreach($devs as $d):
        $ms=milestone((int)$d['learner_count']);
        $cr=$d['_cert_rate'];
    ...
    ?>
      <div class="card<?= $d['_stale']?' stale':'' ?>" data-name="<?= h(strtolower($d['school_name'])) ?>" data-group="<?= h(strtolower($groupName)) ?>">
        <div class="card-title">
          <h3><?= h($d['school_name']?:'(unnamed school)') ?></h3>
          <?php if($ms): ?><span class="milestone-badge" title="<?= $ms==='🏆'?'500+':($ms==='🌳'?'200+':($ms==='🌿'?'100+':'25+')) ?> learners"><?= $ms ?></span><?php endif; ?>
        </div>
        <div class="sub">
        ...
        <div class="metric"><span class="lbl">👨‍🎓 Learners</span><span class="val"><?= num($d['learner_count']) ?>
          <?php if($d['learner_count_prev']!==null):$delta=(int)$d['learner_count']-(int)$d['learner_count_prev'];if($delta>0):?><span style="color:#16a34a;font-size:.78rem"> +<?= $delta ?></span><?php elseif($delta<0):?><span style="color:#dc2626;font-size:.78rem"> <?= $delta ?></span><?php endif;endif;?></span></div>
        <div class="metric"><span class="lbl">📚 Lessons done</span><span class="val"><?= num($d['lesson_completions']) ?></span></div>
        <?php if((int)$d['cert_count']>0):?>
        ...
        <?php endif;?>
        <?php if(!empty($d['avg_sync_interval_secs'])):$mins=round($d['avg_sync_interval_secs']/60,1);$col=$mins<=10?'#166534':($mins<=60?'#92400e':'#991b1b');?>
...
<section class="groups hidden" id="view-regions">
  ...
  <div class="rollup-card">
    <h3>📍 <?= h($rName) ?> <span style="font-size:.8rem;color:#6b7280;font-weight:400;"><?= $rl['devices'] ?> school<?= $rl['devices']!==1?'s':'' ?> · <?= $rl['online'] ?> online</span></h3>
    <div class="rollup-stats">
      <div class="rs"><div class="rv"><?= num($rl['learners']) ?></div><div class="rl">Learners</div></div>
...
<footer>CBE LMS · live data from school LAN boxes · updates on cron</footer>
...
(function(){
  ...
  markers.forEach(function(m){
    var r=m.learners>=500?13:(m.learners>=200?11:(m.learners>=100?10:(m.learners>=25?9:8)));
    var sc=m.avg_score;
    ...
    var rows=[
      ...
      ['👨‍🎓 Learners',fmt(m.learners)+(m.milestone?' '+m.milestone:'')+(m.learner_delta>0?' <span style="color:#16a34a">+'+m.learner_delta+'</span>':(m.learner_delta<0?' <span style="color:#dc2626">'+m.learner_delta+'</span>':''))],
      m.cert_rate!==null?['🎓 Cert rate','<b style="color:#047857">'+pc(m.cert_rate)+'</b> ('+fmt(m.certs)+')']:null,
      ...
      m.app_version?['📦 App',esc(m.app_version)]:null,
    ].filter(Boolean);
    ...
    var color=m.stale?'#9ca3af':'#059669';
