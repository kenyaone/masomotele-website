    ...
    if ($row) {
        ...
        if ($row->enrollment_id) {
            ...
            if ($pay_stmt) {
                ...
                // ── UPDATE mtti_student_balances so learner portal reflects payment ──
                $balances_table   = $table_prefix . 'mtti_student_balances';
                $courses_table_n  = $table_prefix . 'mtti_courses';
                $enrollments_tbl  = $table_prefix . 'mtti_enrollments';
                $payments_tbl_n   = $table_prefix . 'mtti_payments';

                // Get balance record for this enrollment
                $bq = $db->prepare("SELECT * FROM `$balances_table` WHERE enrollment_id = ?");
                $bq->bind_param('i', $eid);
                $bq->execute();
                $bal_result = $bq->get_result();
                $bq->close();
                $bal = $bal_result ? $bal_result->fetch_object() : null;

                if ( $bal ) {
                    // Live course fee
                    $fq = $db->prepare(
                        "SELECT c.fee FROM `$enrollments_tbl` e
                         JOIN `$courses_table_n` c ON e.course_id = c.course_id
                         WHERE e.enrollment_id = ?"
                    );
                    $fq->bind_param('i', $eid);
                    $fq->execute();
                    $fr = $fq->get_result()->fetch_object();
                    $fq->close();
                    $actual_fee = ($fr && floatval($fr->fee) > 0) ? floatval($fr->fee) : floatval($bal->total_fee);

                    // Total paid from confirmed payments for this enrollment (excludes current — status not yet Completed)
                    $pq = $db->prepare(
                        "SELECT COALESCE(SUM(amount),0) FROM `$payments_tbl_n`
                         WHERE enrollment_id = ? AND status = 'Completed'"
                    );
                    $pq->bind_param('i', $eid);
                    $pq->execute();
                    $prev_paid = floatval($pq->get_result()->fetch_row()[0]);
                    $pq->close();
                    $real_total_paid = $prev_paid + $amt; // add current payment

                    $discount  = floatval($bal->discount_amount ?? 0);
                    $new_bal   = max(0, $actual_fee - $discount - $real_total_paid);
                    $now       = date('Y-m-d H:i:s');

                    $uq = $db->prepare(
                        "UPDATE `$balances_table`
                         SET total_fee=?, total_paid=?, balance=?, last_payment_date=?
                         WHERE balance_id=?"
                    );
                    $uq->bind_param('dddsi', $actual_fee, $real_total_paid, $new_bal, $now, $bal->balance_id);
                    $uq->execute();
                    $uq->close();
                    log_ncba('BALANCE_UPDATED', $trans_id, $amt, $bill_ref,
                        "enrollment $eid | paid=$real_total_paid | bal=$new_bal");
                } else {
                    log_ncba('BALANCE_MISSING', $trans_id, $amt, $bill_ref,
                        "No student_balances row for enrollment $eid — balance NOT updated");
                }
                // ── END BALANCE UPDATE ──────────────────────────────────────────
            }
        }
