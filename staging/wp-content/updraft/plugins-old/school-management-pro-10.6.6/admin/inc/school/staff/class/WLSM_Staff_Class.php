				...
				if (count($filter_rows_limit)) {
					foreach ($filter_rows_limit as $row) {
						...
						$data[] = array(
							...
							esc_html(
								WLSM_M_Class::get_label_text($row->class_label),
							...
							(!empty($row->attachments) ? '<a class="btn btn-primary btn-sm ml-3" href="' . WLSM_M_Staff_Class::get_homework_submission_link($row->attachments) . '"> <i class="fas fa-file-download"></i></a>' : ''),
