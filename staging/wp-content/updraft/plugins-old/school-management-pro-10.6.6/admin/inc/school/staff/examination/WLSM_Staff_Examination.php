	...
	public static function fetch_academic_report()
	{
		...
		if ($assigned_class) {
			...
			if ($restrict_to_section) {
				$restrict_to_section_detail = WLSM_M_Staff_Class::get_section_by_id($restrict_to_section);
				$class_id = $restrict_to_section_detail->class_id;
				$query = WLSM_M_Staff_Examination::fetch_academic_report_query_by_class_id($school_id, $class_id);
				$rows_query = WLSM_M_Staff_Examination::fetch_academic_report_query_count_by_class_id($school_id, $class_id);
			} else {
				$query = WLSM_M_Staff_Examination::fetch_academic_report_query($school_id) . ' AND 1=0';
				$rows_query = WLSM_M_Staff_Examination::fetch_academic_report_query_count($school_id) . ' AND 1=0';
			}
		} else {
			// Total query.
	...
	public static function save_exam_results()
	{
		...
		if (count($errors) < 1) {
			try {
				...
				// Check if all subject results are completed for this student and trigger SMS notification
