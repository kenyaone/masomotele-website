				...
				if ($student_id && isset($student) && !$is_insert) {
					...
					if ($old_route_vehicle_id != $new_route_vehicle_id && !empty($new_route_vehicle_id)) {
						...
						if ($current_route_vehicle) {
							// Check if transport invoice already exists for this route vehicle
							$existing_transport_invoice = $wpdb->get_var($wpdb->prepare(
								"SELECT COUNT(*) FROM " . WLSM_INVOICES . "
								 WHERE student_record_id = %d AND label = 'Transport Fee'",
								$student_id
							));

							// Only create if no transport invoice exists yet
							if ($existing_transport_invoice == 0) {
