...
class WLSM_M_Staff_Examination
{
	...
	public static function fetch_exam_groups_by_ids($school_id, $group_ids)
	{
		...
		$query = $wpdb->prepare(
			"SELECT eg.ID, eg.label FROM " . WLSM_EXAMS_GROUP . " AS eg
			WHERE eg.school_id = %d AND eg.ID IN (" . $placeholders . ")
			ORDER BY FIELD(eg.ID, " . implode(', ', $group_ids) . ")",
			array_merge(array($school_id), $group_ids)
		);
	...
	public static function fetch_academic_report_query_by_class_id($school_id, $class_id)
	{
		global $wpdb;
		...
		$query = $wpdb->prepare(
			...
			INNER JOIN " . WLSM_CLASSES . " AS c ON ar.class_id = c.ID
			INNER JOIN " . WLSM_EXAMS_GROUP . " AS eg ON ar.exam_group = eg.ID
			WHERE ar.school_id = %d AND c.ID = %d",
			$school_id,
			$class_id
		);
	...
	public static function fetch_academic_report_query_count_by_class_id($school_id, $class_id)
	{
		$query = 'SELECT COUNT(DISTINCT ar.ID) FROM ' . WLSM_ACADEMIC_REPORTS . ' as ar
		INNER JOIN ' . WLSM_CLASSES . ' as c ON ar.class_id = c.ID
		WHERE ar.school_id = ' . absint($school_id) . ' AND c.ID = ' . absint($class_id);
		return $query;
	}
	...
	public static function get_class_school_exams_academic_report($school_id, $class_school_id, $academic_report_id)
	{
		...
		$exams = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT DISTINCT ex.ID, ex.label AS exam_title, ex.show_rank, ex.show_remark, ex.psychomotor_analysis, ex.psychomotor, ex.start_date, ex.end_date, ex.results_obtained_marks, ex.enable_total_marks, ex.teacher_signature, ex.grade_criteria, eg.ID as exam_group
				FROM " . WLSM_EXAMS . " AS ex
	...
	public static function get_exam_result_by_subject_code($school_id, $exam_id, $student_id, $subject_code)
	{
		...
		$exam_result = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT ep.maximum_marks, er.obtained_marks, er.remark, er.scale FROM ' . WLSM_EXAM_RESULTS . ' as er
			JOIN ' . WLSM_EXAM_PAPERS . ' as ep ON ep.ID = er.exam_paper_id
			JOIN ' . WLSM_EXAMS . ' as ex ON ex.ID = ep.exam_id
			JOIN ' . WLSM_ADMIT_CARDS . ' as ac ON ac.ID = er.admit_card_id
			JOIN ' . WLSM_STUDENT_RECORDS . ' as sr ON sr.ID = ac.student_record_id
