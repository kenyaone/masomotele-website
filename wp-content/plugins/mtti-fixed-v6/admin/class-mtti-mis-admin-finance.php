...
class MTTI_MIS_Admin_Finance {
    ...
    public function display() {
        ...
        // ── CSV export for student fee payments ──────────────────────
        if ( ! empty($_GET['export_fees_csv']) && $tab === 'income' ) {
            $rows = $wpdb->get_results($wpdb->prepare(
                "SELECT p.payment_date, p.receipt_number, u.display_name AS student_name,
                        s.admission_number, c.course_name, c.category AS course_category,
                        p.payment_method, p.transaction_reference, p.amount
                 FROM {$this->pay_table} p
                 LEFT JOIN {$wpdb->prefix}mtti_students s ON p.student_id = s.student_id
                 LEFT JOIN {$wpdb->users} u ON s.user_id = u.ID
                 LEFT JOIN {$wpdb->prefix}mtti_enrollments e ON p.enrollment_id = e.enrollment_id
                 LEFT JOIN {$wpdb->prefix}mtti_courses c ON e.course_id = c.course_id
                 WHERE p.status='Completed' AND DATE_FORMAT(p.payment_date,'%%Y-%%m')=%s
                 ORDER BY p.payment_date DESC", $month
            ));
            $filename = 'student-fees-' . $month . '.csv';
            header('Content-Type: text/csv');
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            $out = fopen('php://output', 'w');
            fputcsv($out, ['Date','Receipt #','Student Name','Adm. No.','Course','Category','Method','Reference','Amount (KES)']);
            foreach ( $rows as $r ) {
                fputcsv($out, [
                    $r->payment_date, $r->receipt_number, $r->student_name,
                    $r->admission_number, $r->course_name, $r->course_category,
                    $r->payment_method, $r->transaction_reference, $r->amount
                ]);
            }
            fclose($out);
            exit;
        ...
        <?php
        $fee_payments = $wpdb->get_results($wpdb->prepare(
            "SELECT
                p.payment_id,
                p.payment_date,
                p.receipt_number,
             ...
             WHERE p.status = 'Completed'
               AND DATE_FORMAT(p.payment_date,'%%Y-%%m') = %s
        ...
        foreach ( $fee_payments as $fp ) {
            $cn = $fp->course_name ?: 'Unlinked / Direct Payment';
            if ( ! isset($by_course[$cn]) ) $by_course[$cn] = ['count'=>0,'total'=>0,'category'=>$fp->course_category];
            $by_course[$cn]['count']++;
            $by_course[$cn]['total'] += floatval($fp->amount);
        }
        arsort($by_course);
        ?>
        ...
        <div style="margin-top:30px;">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
                <h3 style="margin:0;color:#0073aa;">🎓 Student Fee Payments Breakdown</h3>
                <button class="button" onclick="var el=document.getElementById('sfb-wrap');el.style.display=el.style.display==='none'?'block':'none';">
                    Toggle Details
                </button>
            </div>

            <!-- Course summary mini-table -->
            <table class="wp-list-table widefat fixed" style="font-size:13px;max-width:680px;margin-bottom:16px;">
                <thead>
                    <tr style="background:#0073aa;">
                        <th style="color:#fff;">Course</th>
                        <th style="color:#fff;text-align:center;">Payments</th>
                        <th style="color:#fff;text-align:right;">Amount (KES)</th>
                        <th style="color:#fff;text-align:right;">%</th>
                    </tr>
                </thead>
                <tbody>
                <?php foreach ( $by_course as $cname => $cdata ): ?>
                <tr>
                    <td>
                        <strong><?php echo esc_html($cname); ?></strong>
                        <?php if($cdata['category']): ?>
                        <br><small style="color:#888;"><?php echo esc_html($cdata['category']); ?></small>
                        <?php endif; ?>
                    </td>
                    <td style="text-align:center;"><?php echo $cdata['count']; ?></td>
                    <td style="text-align:right;font-weight:600;color:#0073aa;"><?php echo number_format($cdata['total'],2); ?></td>
                    <td style="text-align:right;color:#777;"><?php echo $fee_total>0 ? round($cdata['total']/$fee_total*100,1) : 0; ?>%</td>
                </tr>
                <?php endforeach; ?>
                </tbody>
                <tfoot>
                    <tr style="background:#f0f7ff;">
                        <td style="font-weight:700;">TOTAL</td>
                        <td style="text-align:center;font-weight:700;"><?php echo $fee_count; ?></td>
                        <td style="text-align:right;font-weight:700;color:#0073aa;">KES <?php echo number_format($fee_total,2); ?></td>
            ...
            <div id="sfb-wrap" style="display:none;">
                <h4 style="margin-bottom:8px;color:#555;">📋 All Transactions (<?php echo $fee_count; ?>)</h4>
                <table class="wp-list-table widefat fixed striped" style="font-size:12px;">
                    <thead>
                        <tr>
                            ...
                            <th width="100">Ref / Code</th>
                            <th width="110" style="text-align:right;">Amount (KES)</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php foreach ( $fee_payments as $fp ): ?>
                    <tr>
                        <td><?php echo esc_html($fp->payment_date); ?></td>
                        <td><code style="font-size:11px;"><?php echo esc_html($fp->receipt_number); ?></code></td>
                        <td><?php echo esc_html($fp->student_name ?: '—'); ?></td>
                        <td style="color:#777;font-size:11px;"><?php echo esc_html($fp->admission_number ?: '—'); ?></td>
                        <td>
                            <?php echo esc_html($fp->course_name ?: '<em>No course linked</em>'); ?>
                            <?php if($fp->course_category): ?>
                            <br><span style="font-size:10px;color:#888;"><?php echo esc_html($fp->course_category); ?></span>
                            <?php endif; ?>
                        </td>
                        <td>
                            <span style="background:#e3f2fd;color:#1565c0;padding:2px 6px;border-radius:3px;font-size:11px;">
                                <?php echo esc_html($fp->payment_method); ?>
                            </span>
                        </td>
                        <td style="font-size:11px;color:#777;"><?php echo esc_html($fp->transaction_reference ?: '—'); ?></td>
                        <td style="text-align:right;font-weight:700;color:#0073aa;"><?php echo number_format(floatval($fp->amount),2); ?></td>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>
                    <tfoot>
                        <tr style="background:#e3f2fd;">
                            <td colspan="7" style="text-align:right;font-weight:700;">TOTAL FEES COLLECTED:</td>
                            <td style="text-align:right;font-weight:700;color:#0073aa;">KES <?php echo number_format($fee_total,2); ?></td>
                        </tr>
                    </tfoot>
                </table>
                ...
                <div style="margin-top:10px;">
                    <a href="<?php echo esc_url(admin_url('admin.php?page=mtti-mis-finance&tab=income&month='.urlencode($month).'&export_fees_csv=1')); ?>"
