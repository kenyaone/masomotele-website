...
class MTTI_MIS_Admin_Payments {
    ...
    private function display_form($payment_id = null) {
        ...
        <div class="wrap">
            ...
            <?php if (!$is_edit): ?>
            <!-- STK Push: Admin-initiated M-Pesa prompt -->
            <div style="background:#e8f5e9;border:1px solid #a5d6a7;border-radius:8px;padding:22px;margin-top:24px;max-width:700px;">
                <h3 style="margin:0 0 6px;color:#2E7D32;">📲 Send M-Pesa STK Push to Student</h3>
                <p style="margin:0 0 16px;color:#555;font-size:13px;">Send a payment prompt directly to the student's phone. Payment will auto-post to their account when confirmed.</p>
                <table class="form-table" style="margin:0;">
                    <tr>
                        <th style="width:160px;"><label>Student ID</label></th>
                        <td><input type="number" id="adm-stk-student-id" placeholder="Student ID" style="width:180px;padding:8px;border:1px solid #ccc;border-radius:4px;"></td>
                    </tr>
                    <tr>
                        <th><label>Phone Number</label></th>
                        <td><input type="tel" id="adm-stk-phone" placeholder="07XXXXXXXX" style="width:180px;padding:8px;border:1px solid #ccc;border-radius:4px;"></td>
                    </tr>
                    <tr>
                        <th><label>Amount (KES)</label></th>
                        <td><input type="number" id="adm-stk-amount" min="1" placeholder="e.g. 5000" style="width:180px;padding:8px;border:1px solid #ccc;border-radius:4px;"></td>
                    </tr>
                </table>
                <div style="margin-top:16px;">
                    <button onclick="adm_stk_push()" id="adm-stk-btn" class="button button-primary" style="background:#2E7D32;border-color:#1b5e20;font-size:14px;height:36px;padding:0 20px;">
                        💳 Send STK Push
                    </button>
                </div>
                <div id="adm-stk-msg" style="margin-top:12px;display:none;padding:10px 14px;border-radius:6px;font-size:13px;"></div>
            </div>
            <script>
            function adm_stk_push() {
                var sid    = document.getElementById('adm-stk-student-id').value;
                var phone  = document.getElementById('adm-stk-phone').value;
                var amount = document.getElementById('adm-stk-amount').value;
                var msg    = document.getElementById('adm-stk-msg');
                var btn    = document.getElementById('adm-stk-btn');
                ...
                fetch('<?php echo esc_url(home_url('/mtti-stk-push.php')); ?>', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({student_id: parseInt(sid), phone: phone, amount: parseInt(amount), source: 'admin'})
                })
                ...
                .then(function(d) {
                    ...
                    if (d.success) {
                        ...
                        msg.innerHTML = '✅ ' + d.message + (d.student_name ? ' — <strong>' + d.student_name + '</strong>' : '');
                ...
                (function() {
                    ...
                    if ($payment->enrollment_id) {
                        ...
                        if ($edit_enrollment) {
                            ...
                            $edit_excluded_courses = array('computer applications', 'computer essentials', 'computer & online essentials', 'ielts');
                            ...
                            if ($edit_requires_additional && isset($edit_enrollment->duration_weeks) && intval($edit_enrollment->duration_weeks) > 0 && intval($edit_enrollment->duration_weeks) <= 8) {
                                $edit_requires_additional = false;
                            }
    ...
    private function update_student_balance($enrollment_id, $payment_amount, $discount_amount = 0) {
        ...
        if ($balance) {
            ...
            // Payment is already saved with status='Completed' before this runs — do NOT add $payment_amount again.
