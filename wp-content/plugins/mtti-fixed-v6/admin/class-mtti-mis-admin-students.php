...
class MTTI_MIS_Admin_Students {
    ...
    public function display() {
        ...
        // Handle disable (discontinue) action
        if ($action == 'disable' && $student_id) {
            check_admin_referer('mtti_disable_student_' . $student_id);
            if (ob_get_length()) {
                ob_end_clean();
            }
            $this->disable_student($student_id);
            return;
        }
        ...
        <div class="wrap">
            ...
            <div class="notice <?php echo ($_GET['message'] == 'created_no_email') ? 'notice-warning' : 'notice-success'; ?> is-dismissible">
                <p>
                    ...
                    } elseif ($_GET['message'] == 'disabled') {
                        echo 'Student has been <strong>discontinued</strong>. They can no longer access the learner portal.';
                    }
                    ?>
                </p>
            </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($student->status !== 'Discontinued') : ?>
                            <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=mtti-mis-students&action=disable&id=' . $student->student_id), 'mtti_disable_student_' . $student->student_id); ?>"
                               onclick="return confirm('Discontinue this student? They will no longer be able to access the learner portal.');"
                               style="color: #b71c1c;">Disable</a> |
                            <?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="status" id="status" class="regular-text">
                                ...
                                <option value="Discontinued" <?php selected($student->status, 'Discontinued'); ?>>Discontinued</option>
                            </select>
                            <?php if ($student->status === 'Discontinued') : ?>
                            <p style="color: #b71c1c; margin-top: 5px;"><strong>⚠ This student is discontinued and cannot access the learner portal.</strong></p>
                            <?php endif; ?>
                        </td>
                    </tr>
                </table>
    ...
    private function disable_student($student_id) {
        global $wpdb;
        $students_table = $this->db->get_table_name('students');

        $wpdb->update(
            $students_table,
            array('status' => 'Discontinued'),
            array('student_id' => $student_id),
            array('%s'),
            array('%d')
        );

        $redirect_url = admin_url('admin.php?page=mtti-mis-students&message=disabled');
        if (!headers_sent()) {
            wp_redirect($redirect_url);
            exit;
        } else {
