...
class MTTI_MIS_Admin {
    ...
    /**
     * Ensures critical capabilities are always present on the correct roles.
     * Runs on every admin_init so no save/reactivation can permanently strip them.
     */
    public function ensure_required_caps() {
        // Administrator always gets all MTTI caps
        $admin = get_role('administrator');
        if ($admin) {
            foreach (['manage_finance', 'manage_assets', 'publish_lessons', 'manage_mtti',
                      'manage_students', 'manage_courses', 'manage_enrollments',
                      'manage_payments', 'manage_staff', 'manage_attendance',
                      'manage_assessments', 'manage_assignments', 'manage_live_classes',
                      'manage_certificates', 'manage_notices', 'schedule_exams'] as $cap) {
                $admin->add_cap($cap);
            }
        }
        // Teacher always gets publish_lessons
        $teacher = get_role('mtti_teacher');
        if ($teacher) {
            $teacher->add_cap('publish_lessons');
        }
        // Systems admin gets everything teacher gets plus more
        $sys_admin = get_role('mtti_systems_admin');
        if ($sys_admin) {
            $sys_admin->add_cap('publish_lessons');
        }
    }
    ...
    public function ajax_get_student_fee_info() {
        ...
        $sql = $wpdb->prepare(
            "SELECT e.*, c.course_name, c.course_code, c.fee as course_fee, c.duration_weeks
        ...
        $excluded_courses = array('computer applications', 'computer essentials', 'computer & online essentials', 'ielts', 'application');
        ...
        foreach ($enrollments as $enrollment) {
            ...
            // Courses 8 weeks or shorter (≤ 2 months) are exempt from admission fee
            if ($course_requires_fees && isset($enrollment->duration_weeks) && intval($enrollment->duration_weeks) > 0 && intval($enrollment->duration_weeks) <= 8) {
                $course_requires_fees = false;
            }
    ...
    public function enqueue_scripts() {
        ...
        // Chart.js for analytics dashboard and course income report
        $chartjs_pages = ['mtti-mis', 'mtti-mis-course-income-report'];
        if (!isset($_GET['page']) || in_array($_GET['page'], $chartjs_pages)) {
    ...
    public function add_plugin_admin_menu() {
        ...
        // Course Income Report submenu
        add_submenu_page(
            'mtti-mis',
            'Course Income Report',
            '📈 Course Income',
            'manage_payments',
            'mtti-mis-course-income-report',
            array($this, 'display_course_income_report')
        );
    ...
    public function display_course_income_report() {
    ...
    public function display_settings() {
        ...
        // Handle stamp / signature image upload
        if (isset($_POST['mtti_save_admission_images']) && check_admin_referer('mtti_admission_images', 'mtti_images_nonce')) {
            foreach (array('mtti_stamp_image' => 'mtti_admission_stamp_b64', 'mtti_signature_image' => 'mtti_admission_signature_b64') as $field => $option) {
                if (!empty($_FILES[$field]['tmp_name']) && !$_FILES[$field]['error']) {
                    $mime    = mime_content_type($_FILES[$field]['tmp_name']);
                    $allowed = array('image/jpeg', 'image/png', 'image/gif', 'image/webp');
                    if (in_array($mime, $allowed)) {
                        $raw     = file_get_contents($_FILES[$field]['tmp_name']);
                        $cropped = $this->trim_image_whitespace($raw);
                        update_option($option, 'data:image/png;base64,' . base64_encode($cropped));
                    }
                }
            }
            // Handle clear buttons
            if (!empty($_POST['mtti_clear_stamp']))     delete_option('mtti_admission_stamp_b64');
            if (!empty($_POST['mtti_clear_signature'])) delete_option('mtti_admission_signature_b64');
            echo '<div class="notice notice-success is-dismissible"><p>Admission letter images saved and auto-cropped.</p></div>';
        }
        ...
        <div class="wrap">
            ...
            <h2 class="nav-tab-wrapper">
                ...
                <a href="?page=mtti-mis-settings&tab=admission_images" class="nav-tab <?php echo $active_tab == 'admission_images' ? 'nav-tab-active' : ''; ?>">
                    🖋️ Admission Letter Images
                </a>
            ...
            <?php elseif ($active_tab == 'admission_images') : ?>
            <!-- Admission Letter Images Tab -->
            <p style="margin:12px 0 6px; color:#555;">Upload the official <strong>stamp/seal</strong> and <strong>principal's signature</strong> that will appear on the admission letter PDF. PNG with a transparent background works best.</p>
            <form method="post" enctype="multipart/form-data" action="">
                <?php wp_nonce_field('mtti_admission_images', 'mtti_images_nonce'); ?>
                <input type="hidden" name="mtti_save_admission_images" value="1">
                <table class="form-table">
                    <tr>
                        <th scope="row">Official Stamp / Seal</th>
                        <td>
                            <?php $stamp = get_option('mtti_admission_stamp_b64'); ?>
                            <?php if ($stamp): ?>
                            <div style="margin-bottom:10px;">
                                <img src="<?php echo esc_attr($stamp); ?>" style="max-width:160px; max-height:160px; border:1px solid #ddd; padding:6px; background:#fff;">
                                <br>
                                <label style="margin-top:6px; display:inline-flex; align-items:center; gap:6px; cursor:pointer;">
                                    <input type="checkbox" name="mtti_clear_stamp" value="1"> Remove current stamp
                                </label>
                            </div>
                            <?php endif; ?>
                            <input type="file" name="mtti_stamp_image" accept="image/png,image/jpeg,image/gif,image/webp">
                            <p class="description">PNG with transparent background recommended. Max ~1 MB.</p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">Principal's Signature</th>
                        <td>
                            <?php $sig = get_option('mtti_admission_signature_b64'); ?>
                            <?php if ($sig): ?>
                            <div style="margin-bottom:10px;">
                                <img src="<?php echo esc_attr($sig); ?>" style="max-width:220px; max-height:80px; border:1px solid #ddd; padding:6px; background:#fff;">
                                <br>
                                <label style="margin-top:6px; display:inline-flex; align-items:center; gap:6px; cursor:pointer;">
                                    <input type="checkbox" name="mtti_clear_signature" value="1"> Remove current signature
                                </label>
                            </div>
                            <?php endif; ?>
                            <input type="file" name="mtti_signature_image" accept="image/png,image/jpeg,image/gif,image/webp">
                            <p class="description">Signature image on white or transparent background. Will appear above the "Principal" line on the PDF.</p>
                        </td>
                    </tr>
                </table>
                <?php submit_button('Save Admission Letter Images'); ?>
            </form>
    ...
    private function display_role_permissions() {
        ...
        $mtti_capabilities = array(
            ...
            'publish_lessons' => array(
                'label' => 'Publish / Unpublish Lessons & Interactives',
                'description' => 'Lecturer can publish, unpublish, and manage their own lessons and interactive content'
            ),
            'upload_student_photo' => array(
                'label' => 'Upload Student Photos',
                'description' => 'Upload and change passport photos on student profiles'
            ),
        );
        ...
        function resetToDefaults() {
            if (confirm('Reset all role permissions to defaults? This will restore the original permission settings.')) {
                ...
                var defaults = {
                    ...
                    'mtti_teacher': ['manage_mtti', 'manage_students', 'view_students', 'manage_courses', 'manage_assignments', 'manage_live_classes', 'manage_notices', 'manage_attendance', 'manage_assessments', 'publish_lessons'],
                    'mtti_registrar': ['manage_mtti', 'manage_students', 'view_students', 'manage_courses', 'manage_enrollments', 'manage_certificates', 'manage_notices', 'upload_student_photo'],
                    'mtti_accountant': ['manage_mtti', 'manage_students', 'view_students', 'manage_payments', 'manage_enrollments', 'upload_student_photo']
                };
    ...
    private function save_role_permissions() {
        ...
        // MTTI capabilities to manage — must mirror display_role_permissions() list exactly
        $mtti_capabilities = array(
            'manage_mtti',
            'manage_students',
            'view_students',
            ...
            'manage_finance',
            'manage_assets',
            'publish_lessons',       // lecturer publish/unpublish
        );
    ...
    private function trim_image_whitespace($raw_data) {
        ...
        if (!$src) return $raw_data;

        $w = imagesx($src);
        $h = imagesy($src);

        // Convert to RGBA so we can detect transparent pixels too
