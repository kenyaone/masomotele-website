...
/**
 * Publish or unpublish a lesson/interactive from the lecturer portal.
 * @param {number} lessonId
 * @param {string} newStatus  'Published' or 'Draft'
 * @param {HTMLElement} btn   The button that was clicked
 */
function mttiToggleLesson(lessonId, newStatus, btn) {
    btn.disabled = true;
    btn.textContent = '...';
    jQuery.post(mttiLecturer.ajaxUrl, {
        action:    'mtti_lecturer_toggle_lesson',
        nonce:     mttiLecturer.nonce,
        lesson_id: lessonId,
        status:    newStatus,
    }, function(res) {
        if (res.success) {
            ...
            alert(res.data || 'Could not update lesson status.');
            btn.disabled = false;
            btn.textContent = newStatus === 'Published' ? '⬆ Publish' : '⬇ Unpublish';
        }
    }).fail(function() {
