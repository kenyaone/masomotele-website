...
class MTTI_MIS_Learner_Portal {
    ...
    public function __construct() {
        ...
        add_action('template_redirect', array($this, 'maybe_render_fullpage'), 1);
    ...
    public function maybe_render_fullpage() {
        ...
        if (!$is_portal && isset($post) && $post && is_a($post, 'WP_Post')) {
        ...
        // If this is the student portal page, render it as a fullpage and exit
        if ($is_portal) {
            ...
            // Set status to 200 to show page found
            status_header(200);
    ...
    private function render_fullpage_portal() {
        ...
        if (!is_user_logged_in()) {
            ...
            if (!$student) {
                ...
                $portal_html = $this->render_discontinued_block($student);
            } elseif ($this->student_has_certificate($student->student_id)) {
                ...
                echo '</main></div>';
                echo $this->render_pdf_modal_markup();
                echo '</div>';
...
<script>
// Force fresh load on shared computers — prevents previous student's session showing
(function() {
...
(function(){
    ...
    private function get_current_student() {
        ...
        return $wpdb->get_row($wpdb->prepare(
            "SELECT s.student_id, s.user_id, s.admission_number, s.course_id, s.id_number,
                    ...
                    c.course_id AS course_id, c.course_name, c.course_code,
                    c.fee, c.duration_weeks, c.description AS course_description
             FROM {$table} s
    ...
    public function render_portal($atts = array()) {
        // This is a fallback - the fullpage render on template_redirect should handle it
        // If we get here, just return empty
        return '';
    }

    // Old render_portal logic kept for reference (now unused)
    private function render_portal_deprecated($atts = array()) {
        ...
        if ($student->status === 'Discontinued') {
            return $this->render_discontinued_block($student);
        }
            ...
            if (wp_verify_nonce($_POST['mtti_login_nonce'], 'mtti_portal_login_action')) {
                ...
                if (strpos($username, '/') !== false) {
                    ...
                    $student = $wpdb->get_row($wpdb->prepare(
                        "SELECT s.user_id, s.student_id, s.admission_number, s.status, a.password_hash, a.account_id
                    ...
                    } elseif ($student && $student->status === 'Discontinued') {
                        $error = 'Your account has been discontinued. Please contact the administration office.';
    ...
    private function render_discontinued_block($student): string {
        $name = esc_html($student->display_name ?: 'Student');
        $admission = esc_html($student->admission_number ?? '');
        return '<div class="mtti-portal-not-enrolled">
            <div class="mtti-message-card" style="border-top: 4px solid #B71C1C;">
                <div class="mtti-message-icon">🚫</div>
                <h2 style="color:#B71C1C;">Access Disabled</h2>
                <p>Hello <strong>' . $name . '</strong>' . ($admission ? ' (' . $admission . ')' : '') . ',</p>
                <p>Your account has been <strong>discontinued</strong>. You no longer have access to the learner portal.</p>
                <p style="margin-top:10px; color:#555;">If you believe this is an error, please contact the institution\'s administration office.</p>
                <p style="margin-top:16px;">
                    <a href="' . wp_logout_url(get_permalink()) . '" style="color:#1976D2;">Log out</a>
                </p>
            </div>
        </div>';
    }
    ...
    /**
     * Returns true ONLY if the student has at least one certificate AND
     * every one of their active/enrolled courses already has a Valid certificate.
     *
     * Logic:
     *  - If the student has NO certificates at all  → false (no block).
     *  - If the student has at least one active enrollment that is NOT yet
     *    covered by a Valid certificate           → false (allow portal access
     *    so they can progress in the new course).
     *  - Only when every active/enrolled course is certificated AND at least
     *    one cert exists                           → true  (block portal).
     */
    private function student_has_certificate(int $student_id): bool {
        global $wpdb;

        // 1. Quick bail: no certs at all → nothing to block.
        $cert_count = intval( $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}mtti_certificates WHERE student_id = %d AND status = 'Valid'",
            $student_id
        ) ) );
        if ( $cert_count === 0 ) {
            return false;
        ...
        // 2. Count active enrollments that do NOT yet have a Valid certificate.
        //    If any such enrollment exists the student is still working on
        //    at least one course and should have portal access.
        $uncertified = intval( $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*)
             FROM {$wpdb->prefix}mtti_enrollments e
             LEFT JOIN {$wpdb->prefix}mtti_certificates c
        ...
        // Block only when every active course is already certificated.
        return $uncertified === 0;
    }

    /**
     * Renders the "course completed — portal access closed" page.
     * Shows certificate info and a logout link. No further portal access.
     */
    private function render_certificate_completed_block($student): string {
        global $wpdb;

        $certs = $wpdb->get_results($wpdb->prepare(
            "SELECT certificate_number, verification_code, course_name, grade, completion_date, issue_date
             FROM {$wpdb->prefix}mtti_certificates
             WHERE student_id = %d
        ...
        $cert_rows = '';
        foreach ($certs as $c) {
            $verify_url = home_url('/verify-certificate/?cert=' . urlencode($c->certificate_number) . '&code=' . urlencode($c->verification_code));
            $cert_rows .= '
            <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:18px 22px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
                <div>
                    <div style="font-size:15px;font-weight:700;color:#166534;">🎓 ' . esc_html($c->course_name) . '</div>
                    <div style="font-size:12px;color:#15803d;margin-top:4px;">
                        Grade: <strong>' . esc_html($c->grade) . '</strong>
                        &nbsp;·&nbsp; Cert No: <code style="background:#dcfce7;padding:2px 6px;border-radius:4px;">' . esc_html($c->certificate_number) . '</code>
                        &nbsp;·&nbsp; Issued: ' . date('d M Y', strtotime($c->issue_date)) . '
                    </div>
                </div>
                <a href="' . esc_url($verify_url) . '" target="_blank"
                   style="background:#16a34a;color:white;padding:8px 16px;border-radius:6px;text-decoration:none;font-size:13px;font-weight:600;white-space:nowrap;">
                   🔍 Verify Certificate
                </a>
            </div>';
        }
        ...
        return '
        <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%);padding:40px 20px;box-sizing:border-box;">
            <div style="background:white;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.10);max-width:640px;width:100%;padding:40px 36px;text-align:center;">

                <div style="font-size:72px;margin-bottom:12px;">🏆</div>
                <h1 style="font-size:26px;font-weight:800;color:#14532d;margin:0 0 8px;">
                    Congratulations, ' . esc_html($student->first_name) . '!
                </h1>
                <p style="font-size:15px;color:#4b5563;margin:0 0 28px;line-height:1.6;">
                    You have successfully completed all your enrolled courses and your certificate(s) have been issued.<br>
                    <strong style="color:#166534;">Portal access is closed because you have no active courses remaining.</strong>
                </p>

                <div style="text-align:left;margin-bottom:28px;">' . $cert_rows . '</div>

                <div style="background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:14px 18px;margin-bottom:24px;font-size:13px;color:#92400e;text-align:left;">
                    📞 If you need to re-enrol or access records, please contact administration at
                    <a href="mailto:' . esc_attr($admin_email) . '" style="color:#92400e;font-weight:600;">' . esc_html($admin_email) . '</a>
                </div>

                <a href="' . esc_url($logout_url) . '"
                   style="display:inline-block;background:#dc2626;color:white;padding:12px 28px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:700;">
                   🚪 Log Out
                </a>
            </div>
        </div>';
    ...
    private function render_notices($student) {
        ...
        $active_cat = isset($_GET['notice_cat']) ? sanitize_text_field($_GET['notice_cat']) : 'All';
        $categories = ['All', 'General', 'Academic', 'Fees', 'Event', 'Urgent'];

        $where_cat = $active_cat !== 'All' ? $wpdb->prepare(" AND category = %s", $active_cat) : '';
        ...
        foreach ($categories as $cat) {
            ...
            $url = add_query_arg(['portal_tab' => 'notices', 'notice_cat' => $cat], get_permalink());
            $cat_colors = [
                'All'      => '#607D8B',
                'General'  => '#607D8B',
                'Academic' => '#1976D2',
            ...
            $bg    = $is_active ? $color : '#fff';
            $fc    = $is_active ? '#fff' : $color;
            $border = "1px solid $color";
            echo '<a href="' . esc_url($url) . '" style="padding:7px 18px;border-radius:20px;font-size:13px;font-weight:600;text-decoration:none;background:' . $bg . ';color:' . $fc . ';border:' . $border . ';">' . esc_html($cat) . '</a>';
        }
        echo '</div>';
        ...
        if (!empty($notices)) {
            ...
            foreach ($notices as $n) {
                ...
                $cat_color = $cat_colors[$n->category] ?? '#607D8B';
                echo '<div class="mtti-notice-card priority-' . $pc . '">';
                echo '<div class="mtti-notice-header">';
                echo '<span class="mtti-notice-category" style="background:' . $cat_color . ';color:#fff;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700;">' . esc_html($n->category) . '</span>';
                if ($n->priority === 'High' || $n->priority === 'Urgent') {
                    echo '<span class="mtti-priority-badge" style="background:#D32F2F;color:#fff;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700;margin-left:6px;">🔴 ' . esc_html($n->priority) . '</span>';
                }
                echo '</div>';
            ...
            echo '<div class="mtti-empty-state"><span>🔔</span><h3>No ' . esc_html($active_cat === 'All' ? '' : $active_cat . ' ') . 'Notices</h3><p>Check back later.</p></div>';
        }
        echo '</div>';
    }
    ...
    public function ajax_submit_assignment() {
        ...
        if ($student->status === 'Discontinued') {
            wp_send_json_error(array('message' => 'Your account has been discontinued.'));
        }
            ...
            foreach ($by_course as $ckey => $course_data) {
                ...
                if (!empty($unit_data['materials'])) {
                    ...
                    foreach ($unit_data['materials'] as $m) {
                        ...
                        echo '</div></div>';
                        if (strtolower($m->file_type) === 'pdf') {
                            echo '<button class="mtti-open-pdf mtti-btn mtti-btn-secondary" data-pdf-url="' . esc_attr($m->file_url) . '" data-pdf-title="' . esc_attr($m->title) . '" style="padding:6px 14px;font-size:12px;flex-shrink:0;">📕 View PDF</button>';
                        } else {
...
function mttiSubmitAttCode(nonce) {
    ...
    private function render_materials($student) {
        ...
        if (!empty($materials)) {
            ...
            foreach ($materials as $m) {
                ...
                echo '</div>';
                if (strtolower($m->file_type) === 'pdf') {
                    echo '<button class="mtti-open-pdf mtti-btn mtti-btn-primary" data-pdf-url="' . esc_attr($m->file_url) . '" data-pdf-title="' . esc_attr($m->title) . '" style="width:100%;margin-top:15px;justify-content:center;">📕 View PDF</button>';
                    echo '<a href="' . esc_url($m->file_url) . '" style="display:block;text-align:center;font-size:12px;color:#666;margin-top:8px;text-decoration:none;" download>📥 Download instead</a>';
                } else {
     ...
     */
    private function render_pdf_modal_markup(): string {
        return '
<div id="mtti-pdf-modal" style="display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.88);flex-direction:column;" role="dialog" aria-modal="true" aria-label="PDF Viewer">
  <div style="background:#1e1e2e;padding:10px 16px;display:flex;align-items:center;gap:12px;flex-shrink:0;">
    <span id="mtti-pdf-title" style="flex:1;color:#fff;font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></span>
    <a id="mtti-pdf-download" href="#" download style="color:#93c5fd;font-size:13px;text-decoration:none;white-space:nowrap;">&#x1F4E5; Download</a>
    <button id="mtti-pdf-close" style="background:none;border:none;color:#fff;font-size:26px;line-height:1;cursor:pointer;padding:0 4px;" title="Close">&times;</button>
  </div>
  <iframe id="mtti-pdf-frame" src="" style="flex:1;border:none;width:100%;background:#525659;" allowfullscreen></iframe>
</div>
<script>
(function(){
  function openPdf(url, title) {
    var modal = document.getElementById("mtti-pdf-modal");
    document.getElementById("mtti-pdf-frame").src = url + "#toolbar=1&navpanes=1&view=FitH";
    document.getElementById("mtti-pdf-title").textContent = title || url.split("/").pop();
    var dl = document.getElementById("mtti-pdf-download");
    dl.href = url;
  ...
  function closePdf() {
    ...
    document.getElementById("mtti-pdf-frame").src = "";
    document.body.style.overflow = "";
  }
  document.addEventListener("click", function(e) {
    var btn = e.target.closest(".mtti-open-pdf");
    if (btn) { e.preventDefault(); openPdf(btn.dataset.pdfUrl, btn.dataset.pdfTitle || ""); return; }
    if (e.target.id === "mtti-pdf-close" || e.target.id === "mtti-pdf-modal") closePdf();
  });
  document.addEventListener("keydown", function(e) {
    ...
    public function ajax_post_discussion() {
        ...
        $student = $this->get_current_student();
        if (!$student) wp_send_json_error();
        if ($student->status === 'Discontinued') wp_send_json_error(array('message' => 'Your account has been discontinued.'));
        ...
        foreach ($units as $i => $u) {
            ...
            if (!empty($unit_materials)) {
                ...
                foreach ($unit_materials as $m) {
                    ...
                    if (strtolower($m->file_type) === 'pdf') {
                        echo '<button class="mtti-open-pdf" data-pdf-url="' . esc_attr($m->file_url) . '" data-pdf-title="' . esc_attr($m->title) . '" style="font-size:11px;color:var(--mtti-primary);background:none;border:none;cursor:pointer;padding:0;">View</button>';
                    } else {
    ...
    public function ajax_learner_mark_attendance() {
        ...
        $student = $wpdb->get_row($wpdb->prepare(
            "SELECT s.student_id, s.status AS student_status, e.enrollment_id
             FROM {$p}students s
        ...
        if ($student->student_status === 'Discontinued') {
            wp_send_json_error('Your account has been discontinued.');
        }
